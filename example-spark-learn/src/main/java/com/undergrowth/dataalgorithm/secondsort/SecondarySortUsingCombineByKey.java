package com.undergrowth.dataalgorithm.secondsort;

// STEP-0: import required Java/Spark classes.

import com.undergrowth.dataalgorithm.DataStructures;
...
/**
 * SecondarySortUsingCombineByKey class implements the secondary sort design pattern by using combineByKey().
 *
 *
 * Input:
 *
 * name, time, value x,2,9 y,2,5 x,1,3 y,1,7 y,3,1 x,3,6 z,1,4 z,2,8 z,3,7 z,4,0 p,1,10 p,3,60 p,4,40 p,6,20
 *
 * Output: generate a time-series looking like this:
 *
 * t1   t2   t3   t4  t5     t6 x => [3,  9,   6] y => [7,  5,   1] z => [4,  8,   7,   0] p => [10, null, 60, 40, null , 20]
 *
 * x => [(1,3), (2,9), (3,6)]            where 1 < 2 < 3 y => [(1,7), (2,5), (3,1)]            where 1 < 2 < 3 z => [(1,4), (2,8), (3,7), (4,0)] where
 * 1 < 2 < 3 < 4 p => [(1,10), (3,60), (4,40), (6,20)] where 1 < 3 < 4 < 6
 *
...
public class SecondarySortUsingCombineByKey {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String outputPath = PathAppendUtil.pathAppend(args[1],SecondarySortUsingCombineByKey.class.getSimpleName()) ;
        System.out.println("outputPath=" + outputPath);

        // STEP-2: Connect to the Sark master by creating JavaSparkContext object
        final JavaSparkContext ctx = SparkUtil.createJavaSparkLocalContext(SecondarySortUsingCombineByKey.class.getSimpleName());
